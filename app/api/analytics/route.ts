...
export async function GET(req: NextRequest) {
  ...
  try {
    const [currentRes, prevRes, sparklineRes] = await Promise.all([
      ...
      // Previous period totals (same length, directly before)
      client.query(
        `SELECT
           COALESCE(SUM(views),  0)::bigint                        AS total_views,
           COALESCE(SUM(reach),  0)::bigint                        AS total_reach,
      ...
      // Daily timeseries buckets (used for sparklines + charts)
      client.query(
        `SELECT
